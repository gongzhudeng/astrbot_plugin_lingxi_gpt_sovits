...
- 新增管理员命令 `语音概率 [0~1]` / `tts_probability [0~1]`，可在聊天中查看或设置主动转语音概率。
- 情绪条目新增启用开关；关闭后保留参数，但不会参与关键词匹配、LLM 情绪判断或手动指定。

### 优化

- 角色保存与切换会保留每个情绪条目的启用状态。
- 补充 LLM 语音工具的适用场景说明。
...
- 工具直发语音后透传原始语音文本，避免 Agent 因无文本 completion 误判失败并重复重试。
- 补充空工具参数、空命令和服务层空文本防线的回归测试，确保不会误合成默认文案。

## v1.2.2

### 修复

- 修复中转接口返回连续 JSON 工具参数时无法恢复真实语音文本的问题。
- 拒绝空的语音工具参数，避免误合成默认文案。

...
[![GitHub](https://img.shields.io/badge/作者-灵犀-blue)](https://github.com/gongzhudeng)
...
`灵犀 · GPT-SoVITS 语音合成` 用于把 AstrBot 文本输出转换成语音输出，底层调用 [GPT-SoVITS](https://github.com/RVC-Boss/GPT-SoVITS) 的 API。
...
在 AstrBot 插件市场搜索 `灵犀 · GPT-SoVITS 语音合成` 并安装。
...
路径：`插件管理 -> 灵犀 · GPT-SoVITS 语音合成 -> 操作 -> 插件配置`
...
| `cache.path` | 三种调用方式共用保存目录 | 支持相对/绝对路径；留空默认 `data/plugins_data/灵犀 · GPT-SoVITS 语音合成/audio` |
...
- 特别感谢原作者 [Zhalslar](https://github.com/Zhalslar) 开发的 [astrbot_plugin_GPT_SoVITS](https://github.com/Zhalslar/astrbot_plugin_GPT_SoVITS) 插件，本插件是在其基础上进行修改和优化的。
